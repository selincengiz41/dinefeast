...
fun Food.mapToFoodEntity(isFavorite:Boolean) :FoodEntity{
    return FoodEntity(
        id, category, count, description, imageOne, imageThree, imageTwo, price, rate, saleState, title, salePrice,isFavorite
    )
}
...
fun FoodEntity.mapToFood():Food{
    return Food(category, count, description, id, imageOne, imageThree, imageTwo, price, rate, saleState, title, salePrice)
}